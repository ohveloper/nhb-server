...
const tagHandler = {
    upload: async (req, res, next) => {
        const { tagName, description, tagUrl } = req.body;
        if (!tagName || !description)
            return res.status(400).json({ message: 'Need accurate informations' });
        await tag_1.Tags.create({ tagName, description, tagUrl }).then(d => {
            res.status(201).json({ message: 'New tag is updated' });
        });
    },
    bring: async (req, res, next) => {
        await tag_1.Tags.findAll({ attributes: ['id', 'tagName', 'description', 'tagUrl'] }).then(d => {
